...
namespace Tuntenfisch.Voxels.DC
{
    ...
    public class DualContouring : MonoBehaviour
    {
        ...
        private class Worker : IDisposable
        {
            ...
            private NativeArray<WeightedVertex> m_generatedWeightedVertices;
            ...
            public Status Process()
            {
                if (m_generatedVerticesBuffer0.IsDataAvailable() && m_generatedIndicesBuffer.IsDataAvailable())
                {
                    ...
                    if (requestedVertexCount < VertexCount || requestedIndexCount < IndexCount || m_generatedVerticesBuffer0.HasError || m_generatedIndicesBuffer.HasError)
                    {
                        ...
                        m_generatedVerticesBuffer0.StartReadbackNonAlloc(ref m_generatedWeightedVertices, VertexCount);
                    ...
                    {
                        m_generatedVertices[vertexIndex] = m_generatedWeightedVertices[vertexIndex].ToGPUVertex();
                    }
            ...
            public void GenerateMeshAsync(Task task)
            {
                ...
                m_generatedVerticesBuffer0.StartReadbackNonAlloc(ref m_generatedWeightedVertices, estimatedVertexCount);
            ...
            private void CreateBuffers()
            {
                ...
                if (!m_generatedWeightedVertices.IsCreated || m_generatedWeightedVertices.Length != generatedVerticesCapacity)
                {
                    if (m_generatedWeightedVertices.IsCreated)
                    {
                        m_generatedWeightedVertices.Dispose();
                    }
                    m_generatedWeightedVertices = new NativeArray<WeightedVertex>(generatedVerticesCapacity, Allocator.Persistent);
                }
                ...
                if (m_generatedVerticesBuffer0?.Count != m_generatedVertices.Length)
                {
                    ...
                    m_generatedVerticesBuffer0 = new AsyncComputeBuffer(m_generatedVertices.Length, WeightedVertex.SizeInBytes, ComputeBufferType.Counter);
                }
                ...
                if (m_generatedVerticesBuffer1?.Count != m_generatedVertices.Length)
                {
                    ...
                    // The counter attached to this compute buffer stores the number of vertices generated by dual contouring.
                    m_generatedVerticesBuffer1 = new AsyncComputeBuffer(m_generatedVertices.Length, WeightedVertex.SizeInBytes, ComputeBufferType.Counter);
                }
